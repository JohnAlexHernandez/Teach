...
class ConexionSQLite(context: Context, name: String, factory: Nothing?, version: Int): SQLiteOpenHelper(context,"bdasesoriasacademicas", null, 1) {
    ...
    val clase = "CREATE TABLE CLASE(id_clase INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, fecha TEXT, hora TEXT, duracion INTEGER, id_tutoria, id_estudiante, FOREIGN KEY (id_tutoria) REFERENCES TUTORIA (id_tutoria) ON DELETE CASCADE, FOREIGN KEY (id_estudiante) REFERENCES ESTUDIANTE (id_estudiante) ON DELETE CASCADE);"
